...
const Char = enum(u5) {
    ...
    const letters_bitset: Set = comptime {
        var b: Set = 0;
        var j: u8 = 'a';
        while (j <= 'z') {
    ...
    pub fn writeBitset(writer: anytype, bitset: Set) !void {
        try writer.print("[", .{});
        var i: u5 = 0;
        while (true) {
...
const Nx = struct {
    ...
    const State = struct {
        const num_states = 200; // if @sizeOf(StateSet) > 256, triggers memcpy, gets ~2x slower
        const Set = StateSet(num_states);
        const Index = Set.Index;

        const Branch = struct {
            next_state: Index,
            char_bitset: Char.Set,
        };
    ...
    pub fn printNfa(self: Nx) !void {
        ...
        for (self.states.items) |*s, i| {
            ...
            for (s.branches) |b, j| {
                ...
                try Char.writeBitset(writer, b.char_bitset);
                try writer.print(" -> ", .{});
    ...
    pub fn matchPartial(self: *Nx, input: []const Char, initial_state: State.Index, state_sets: []State.Set) void {
    ...
    pub fn matchFuzzy(self: *Nx, input: []const Char, initial_state_set: State.Set, n_errors: usize) ?usize {
    ...
    fn matchTransition(self: *Nx, char_bitset: Char.Set, start_states: State.Set) State.Set {
...
pub fn main() !void {
    ...
    var i: usize = 0;
    while (i < cycles) {
        var ss = Nx.State.Set.initEmpty();
        ss.set(0);
        ss.setUnion(nx.states.items[0].epsilon_states);
    ...
    var state_sets: [3]Nx.State.Set = .{Nx.State.Set.initEmpty()} ** 3;
    nx.matchPartial(input_chars, 0, &state_sets);
    log("Partial results: {a}\n", .{state_sets});

    log("Sizeof State={}, State.Set={}\n", .{ @sizeOf(Nx.State), @sizeOf(Nx.State.Set) });
}
